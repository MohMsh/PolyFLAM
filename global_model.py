...
class global_model:
    ...
    def __init__(self, models, global_model_, x_test, y_test, current_round, print_model_summary,
    ...
    def aggregate(self):
        ...
        if model_type == "SVC":
            aggregated_model = self.svm_aggregate()
            predicted = self.svm_custom_predict(aggregated_model, self.x_test)
            self.evaluate(predicted) if self.print_model_performance else None
            self.svm_summary(aggregated_model) if self.print_model_summary else None
        elif model_type == "LogisticRegression":
            aggregated_model = self.lr_aggregate()
            predicted = aggregated_model.predict(self.x_test)
            self.evaluate(predicted) if self.print_model_performance else None
            self.lr_summary(aggregated_model) if self.print_model_summary else None
        elif model_type == "GaussianNB":
            aggregated_model = self.gnb_aggregate()
            predicted = aggregated_model.predict(self.x_test)
            self.evaluate(predicted) if self.print_model_performance else None
            self.gnb_summary(aggregated_model) if self.print_model_summary else None
        elif model_type == "SGDClassifier":
            aggregated_model = self.sgd_aggregate()
            predicted = aggregated_model.predict(self.x_test)
            self.evaluate(predicted) if self.print_model_performance else None
            self.sgd_summary(aggregated_model) if self.print_model_summary else None
        elif model_type == "MLPClassifier":
            aggregated_model = self.mlp_aggregate()
            predicted = aggregated_model.predict(self.x_test)
            self.evaluate(predicted) if self.print_model_performance else None
            self.mlp_summary(aggregated_model) if self.print_model_summary else None
        else:
    ...
    def svm_aggregate(self):
        ...
        aggregated_coefficients = [[] for _ in range(self.models[0].dual_coef_.shape[0])]
        aggregated_intercept = 0

        # Combine support vectors, coefficients, and intercepts from each client's model
        for model in self.models:
            aggregated_support_vectors.extend(model.support_vectors_)

            for i in range(model.dual_coef_.shape[0]):
                aggregated_coefficients[i].extend(model.dual_coef_[i])

            aggregated_intercept += model.intercept_

        # Average the intercept
        aggregated_intercept /= len(self.models)
    ...
    def lr_aggregate(self):
        ...
        aggregated_coefficients = np.zeros(self.models[0].coef_.shape)
        aggregated_intercept = 0

        # Combine coefficients and intercepts from each client's model
        for model in self.models:
            aggregated_coefficients += model.coef_
            aggregated_intercept += model.intercept_

        # Average the coefficients and intercept
        aggregated_coefficients /= len(self.models)
        aggregated_intercept /= len(self.models)
    ...
    def gnb_aggregate(self):
        ...
        aggregated_class_priors = np.zeros(self.models[0].class_prior_.shape)
        aggregated_theta = np.zeros(self.models[0].theta_.shape)
        aggregated_sigma = np.zeros(self.models[0].sigma_.shape)

        # Combine class priors, means, and variances from each client's model
        for model in self.models:
            aggregated_class_priors += model.class_prior_
            aggregated_theta += model.theta_
            aggregated_sigma += model.sigma_

        # Average the class priors, means, and variances
        aggregated_class_priors /= len(self.models)
        aggregated_theta /= len(self.models)
        aggregated_sigma /= len(self.models)
        ...
        aggregated_model.classes_ = self.models[0].classes_
    ...
    def sgd_aggregate(self):
        # Initialize the aggregated coefficients and intercept
        aggregated_coefficients = np.zeros(self.models[0].coef_.shape)
        aggregated_intercept = np.zeros(self.models[0].intercept_.shape)
    ...
    def mlp_aggregate(self):
        ...
        aggregated_model = deepcopy(self.models[0])

        # Initialize lists to store the averaged weights and biases of all models
        averaged_coefs = [np.zeros_like(coef) for coef in self.models[0].coefs_]
        averaged_intercepts = [np.zeros_like(intercept) for intercept in self.models[0].intercepts_]
        ...
        dummy_data = np.zeros((2, self.models[0].coefs_[0].shape[0]))
        dummy_target = np.array([0, 1], dtype=int)
        aggregated_model.fit(dummy_data, dummy_target)
